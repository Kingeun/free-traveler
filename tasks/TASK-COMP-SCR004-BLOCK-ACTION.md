# COMP-SCR004-BLOCK-ACTION - 차단·해제+안전 안내 CTA

Category: Component | Implementation Status: IMPLEMENT | Priority: P1

## Context

사용자 차단·해제 버튼과 안전 안내 CTA.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-040)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
- `src/components/mates/BlockAction.tsx`
...
1. 차단 후 해당 사용자의 글/프로필이 더 이상 목록에 보이지 않는다.
2. 차단 해제 시 다시 노출된다.
