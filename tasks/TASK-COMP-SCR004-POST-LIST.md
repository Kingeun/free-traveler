# COMP-SCR004-POST-LIST - 동행글 목록(최대 8개)

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

동행글 목록, 최대 8개 우선 노출. 종료일이 지난 글은 자동으로 마감 표시한다.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-033, REQ-FUNC-037)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
- `src/components/mates/PostList.tsx`
...
1. 종료일이 지난 글에 '마감' 배지가 표시된다(배치 없이 조회 시점 계산).
2. 본문/응답 어디에도 작성자의 이메일·전화번호가 노출되지 않는다.
