...
참가 요청 생성과 작성자의 승인/거절 처리.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-034, REQ-FUNC-035, REQ-FUNC-036, REQ-NF-005)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
- `src/lib/actions/mate-applications.ts`
...
1. 동일 사용자가 같은 글에 중복 PENDING 요청을 만들면 거부된다.
2. 글 작성자가 아닌 사용자가 승인/거절을 시도하면 403이다.
