# API-BLOCKS-REPORTS - 신고·차단 생성 및 Admin 처리

Category: API | Implementation Status: IMPLEMENT | Priority: P0

## Context

신고·차단 생성과 Admin의 신고 상태 처리.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-039, REQ-FUNC-040, REQ-FUNC-041, REQ-FUNC-042, REQ-NF-019)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.

## Requirement Ref

REQ-FUNC-039, REQ-FUNC-040, REQ-FUNC-041, REQ-FUNC-042, REQ-NF-019
...
- `src/lib/actions/blocks-reports.ts`
...
1. 신고 생성은 3초 이내 응답하고 신고 ID를 반환한다.
2. Admin이 아닌 사용자가 신고 상태를 변경하려 하면 거부된다.
