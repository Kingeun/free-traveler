# UNIT-CONTACT-DETECTION - 연락처 패턴 탐지 유닛 테스트

Category: Unit Test | Implementation Status: IMPLEMENT | Priority: P0

## Context

전화번호/이메일/메신저 ID 정규식 탐지 유닛 테스트.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-032)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
1. '010-1234-5678' 포함 텍스트 → 탐지됨.
2. 'abc@example.com' 포함 텍스트 → 탐지됨.
3. 연락처가 없는 일반 문장 → 탐지되지 않음(오탐 최소화 확인).
