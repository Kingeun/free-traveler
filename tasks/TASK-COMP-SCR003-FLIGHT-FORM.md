# COMP-SCR003-FLIGHT-FORM - 항공편 조건 입력(항공 영역)

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

국가/지역/출발일/귀국일 입력 Form. 입력값은 브라우저 상태로만 유지한다.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-011, REQ-FUNC-012, REQ-FUNC-013, REQ-FUNC-015, REQ-FUNC-017, REQ-NF-017)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.

## Requirement Ref

REQ-FUNC-011, REQ-FUNC-012, REQ-FUNC-013, REQ-FUNC-015, REQ-FUNC-017, REQ-NF-017
...
- `src/components/travel-tools/FlightForm.tsx`
...
1. 출발일이 오늘 이전이면 제출이 차단된다.
2. 귀국일이 출발일보다 빠르면 제출이 차단된다.
3. 국가를 바꾸면 기존에 선택된 지역값이 초기화된다.
4. 브라우저 Network 탭에 입력값이 담긴 요청이 없다.
