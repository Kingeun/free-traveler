# COMP-SCR004-REPORT-ACTION - 신고(사유 코드+설명)

Category: Component | Implementation Status: IMPLEMENT | Priority: P1

## Context

사유 코드+설명으로 신고를 제출하는 UI.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-039, REQ-NF-019)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
- `src/components/mates/ReportAction.tsx`
...
1. 신고 제출 시 접수 ID가 화면에 표시된다.
2. 사유 코드를 선택하지 않으면 제출할 수 없다.
