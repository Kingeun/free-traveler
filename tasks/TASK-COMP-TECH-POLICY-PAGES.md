# COMP-TECH-POLICY-PAGES - 이용약관·개인정보처리방침·동행 안전수칙·콘텐츠 면책

Category: Technical Route | Implementation Status: IMPLEMENT | Priority: P2

## Context

이용약관/개인정보처리방침/동행 안전수칙/콘텐츠 면책 정적 페이지.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-080)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
- `.../privacy/page.tsx`
- `.../mate-safety/page.tsx`
- `.../content-disclaimer/page.tsx`
...
1. 4개 정책 페이지가 모두 실제 문구로 채워져 있다(Lorem ipsum 없음).
2. COMP-SCR003-MATE-WRITE-FORM의 동의 링크가 올바른 정책 페이지로 연결된다.
