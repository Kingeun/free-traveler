# COMP-SCR004-FILTER-BAR - 필터(국가/지역/기간/연령대/성별/스타일/모집상태)

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

국가/지역/기간/연령대/성별/스타일/모집상태 필터와 결과 요약.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-030, REQ-NF-004)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
1. 여러 필터를 동시에 적용하면 AND 조건으로 결과가 좁혀진다.
2. 필터 초기화 버튼으로 전체 목록이 복원된다.
