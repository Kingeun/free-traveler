# API-AUTH-PROFILE - 이메일 인증/로그인/프로필/성인확인

Category: API | Implementation Status: IMPLEMENT | Priority: P0

## Context

이메일 인증·로그인·프로필·성인 확인 서버 로직.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-027, REQ-FUNC-028, REQ-FUNC-029, REQ-FUNC-033, REQ-FUNC-066)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.

## Requirement Ref

REQ-FUNC-027, REQ-FUNC-028, REQ-FUNC-029, REQ-FUNC-033, REQ-FUNC-066
...
- `src/lib/actions/auth-profile.ts`
...
1. 성인 확인 시 생년월일 원본이 아니라 `is_adult=true`와 확인 시각만 저장된다.
2. 이메일 인증 전에는 동행 쓰기 권한이 없다.
