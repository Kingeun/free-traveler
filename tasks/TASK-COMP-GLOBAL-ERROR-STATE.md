# COMP-GLOBAL-ERROR-STATE - 재사용 Error State(재시도 포함)

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

여러 화면에서 재사용하는 Error State 블록(오류 메시지+재시도 버튼). Supabase 조회·제출이 실패했을 때 표시한다.
...
1. 네트워크/서버 오류로 조회 또는 제출이 실패하면 오류 메시지와 재시도 버튼이 보인다.
2. 재시도 버튼 클릭 시 동일 요청이 다시 시도된다.
