# COMP-SCR005-MY-ACTIVITY - Member — My Activity(내 글/참가요청/차단/즐겨찾기)

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

내 글/참가 요청/차단 목록/즐겨찾기를 한 곳에 모은 Member 전용 영역.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-040, REQ-FUNC-068)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.

## Requirement Ref

REQ-FUNC-040, REQ-FUNC-068
...
- `src/components/account/MyActivity.tsx`
...
1. 각 목록이 0건일 때 완성형 Empty State(설명+다음 행동 CTA)가 보인다.
2. 즐겨찾기 항목 클릭 시 SCR-001 상세 Drawer로 이동한다.
