# COMP-GLOBAL-TOAST - 인앱 Toast(접수/승인/거절/신고 알림)

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

참가 요청 접수/승인/거절/신고 접수 등 상태 변경을 알리는 인앱 Toast.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-043)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
1. 상태 변경 후 즉시 Toast가 나타난다.
2. 이메일 발송 실패가 있어도 Toast/상태 표시는 정상 동작한다(이메일은 선택 사항).
