...
5개 Screen 공통 상단 Header와 하단 Footer.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-064, REQ-FUNC-079)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.

## Requirement Ref

REQ-FUNC-064, REQ-FUNC-079
...
1. 5개 Screen 모두에서 동일한 Header/Footer 컴포넌트 인스턴스가 렌더링된다.
2. 현재 페이지에 해당하는 nav 항목이 활성 스타일로 표시된다.
3. Mobile에서는 nav가 햄버거 메뉴로 축약된다.
