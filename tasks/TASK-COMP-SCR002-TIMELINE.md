...
여행 Timeline, 최소 6개 시점을 연도·장소·요약과 함께 세로로 표시한다.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-060)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
1. Timeline 항목이 6개 이상이다.
2. 각 항목에 연도/장소/한 줄 요약이 모두 있다.
