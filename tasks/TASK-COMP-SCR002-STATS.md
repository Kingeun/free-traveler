# COMP-SCR002-STATS - 여행 지표

Category: Component | Implementation Status: IMPLEMENT | Priority: P1

## Context

50+ Trips / 30+ Countries 지표 카드.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-057)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
1. 두 지표 값이 SCR-001 FOUNDER-BAND와 동일하다.
