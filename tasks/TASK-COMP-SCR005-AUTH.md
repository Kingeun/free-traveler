# COMP-SCR005-AUTH - Guest — 로그인/가입/비밀번호 재설정

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

Guest 대상 로그인/가입/비밀번호 재설정 Form.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-027, REQ-FUNC-028, REQ-FUNC-066)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
- `src/components/account/Auth.tsx`
...
1. 이메일 인증 없이는 동행 쓰기 권한이 부여되지 않는다.
2. 비밀번호 재설정 이메일 발송 흐름이 동작한다.
