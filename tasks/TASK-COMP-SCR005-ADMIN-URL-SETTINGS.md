# COMP-SCR005-ADMIN-URL-SETTINGS - Admin — 항공·숙소 외부 URL 설정

Category: Component | Implementation Status: IMPLEMENT | Priority: P1

## Context

Admin 전용 항공·숙소 외부 URL 설정 Form.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-077)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
- `src/components/account/AdminUrlSettings.tsx`
...
1. `http://` 또는 `javascript:`/`data:` URL은 저장이 거부된다.
2. 허용목록 밖 도메인은 저장이 거부된다.
...
- http:, javascript:, data: 스킴이나 허용목록 밖 도메인의 저장을 허용하는 것.
