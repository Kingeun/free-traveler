# COMP-SCR004-APPLY-FORM - 참가 요청 Form(500자 이내)+신청 방법 3단계

Category: Component | Implementation Status: IMPLEMENT | Priority: P0

## Context

참가 요청 메시지(500자 이내) 작성 Form과 신청 방법 3단계 안내.

## Project Scope

이 Task가 다루는 모든 Requirement(REQ-FUNC-034, REQ-FUNC-035)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**로 분류되어 있으며 `docs/06_SRS_UIUX_REVISED.md`/`docs/UIUX_TRACEABILITY.md`에서 이 Task ID로 추적된다. EXCLUDED Requirement는 이 Task에 배정되지 않는다.
...
1. 500자를 초과하면 제출이 막힌다.
2. 이미 PENDING/ACCEPTED 상태로 신청한 글에는 중복 신청이 차단된다.
